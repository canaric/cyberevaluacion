...
/**
 * Serverless function for Vercel: logs exam results into a CSV file in a GitHub repo.
 * Requires environment variables set in Vercel:
 * - GH_TOKEN: GitHub Personal Access Token with repo scope
 * - GH_REPO_OWNER: e.g., "canaric"
 * - GH_REPO_NAME: e.g., "cyberevaluacion"
 * - RESULTS_PATH: path to CSV file in repo, e.g., "data/resultados.csv"
 */
  ...
  try {
    ...
    const getResp = await fetch(apiBase, {
      headers: {
        ...
        "User-Agent": "cyberevaluacion-logger"
      }
    });
    ...
    if (getResp.ok) {
      ...
      // Create with header
      csv = "fecha_iso,nombre_apellido,dni,nota_porcentaje,estado\n";
    } else {
      const errTxt = await getResp.text();
      res.status(502).json({ error: "GitHub GET failed", detail: errTxt });
      return;
    }
    ...
    const putBody = {
      message: `log: ${safeName} ${dni} ${status} ${score}%`,
      content: Buffer.from(csv, "utf8").toString("base64"),
      committer: { name: "cyberevaluacion-bot", email: "noreply@example.com" }
    };
    ...
    if (!putResp.ok) {
      ...
      res.status(502).json({ error: "GitHub PUT failed", detail: errTxt });
      return;
    }
